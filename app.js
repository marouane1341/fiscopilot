(() => {
  ...
  // ====== MODULE NORMALISATION (FIX) ======
  function getModuleId(m, idx) {
    // accepte key / id / slug / moduleKey / name / title
    return (
      m?.key ??
      m?.id ??
      m?.slug ??
  ...
  function getModuleTitle(m, idx) {
    return m?.title ?? m?.name ?? `Module ${idx + 1}`;
  }

  function normalizeModules(dbIndex) {
    const raw = dbIndex?.modules ?? dbIndex?.items ?? dbIndex?.data ?? [];
    return (raw || []).map((m, idx) => ({
      id: String(getModuleId(m, idx)),
      title: String(getModuleTitle(m, idx)),
      sources: Array.isArray(m?.sources) ? m.sources : (Array.isArray(m?.files) ? m.files : []),
      _raw: m
    }));
  }

  function findModuleByIdOrTitle(idOrTitle) {
    const needle = String(idOrTitle ?? "").trim();
    if (!needle) return null;

    // 1) match id exact
    let mod = state.modules.find(m => m.id === needle);
    if (mod) return mod;

    // 2) match title exact (case-insensitive)
    const nlow = needle.toLowerCase();
    mod = state.modules.find(m => m.title.toLowerCase() === nlow);
    if (mod) return mod;

    // 3) includes fallback
    mod = state.modules.find(m => m.id.toLowerCase().includes(nlow) || m.title.toLowerCase().includes(nlow));
    return mod || null;
  }
  ...
  function mergePacks(packs) {
    ...
    merged.lessons = merged.lessons.map((x, i) => ({
      id: x.id ?? `lesson_${i + 1}`,
    ...
    merged.cases = merged.cases.map((c, i) => ({
      id: c.id ?? `case_${i + 1}`,
  ...
  function speakText(text) {
    if (!ttsSupported()) return toast("Audio non supporté sur ce navigateur.");
    const clean = String(text ?? "").replace(/\s+/g, " ").trim();
    if (!clean) return toast("Rien à lire.");
    ...
    catch { toast("Audio bloqué par le navigateur."); }
  }
  ...
  function renderDashboard() {
    ...
    el.app.innerHTML = `
      ...
      <div class="list">
        ${modules.map(m => `
          <div class="item" data-open-module="${escapeHtml(m.id)}">
  ...
  function renderModule() {
    if (!state.module || !state.pack) return renderDashboard();

    const lessons = state.pack.lessons ?? [];
    const qcm = state.pack.qcm ?? [];
    const cases = state.pack.cases ?? [];

    const counts = { courses: lessons.length, qcm: qcm.length, cases: cases.length };
    const tab = state.tab;
  ...
  async function loadDbIndex(noCache = false) {
    ...
    state.modules = normalizeModules(state.dbIndex);
  }

  async function openModuleById(idOrTitle) {
    const mod = findModuleByIdOrTitle(idOrTitle);
    if (!mod) {
      toast("Module introuvable.");
      console.warn("Module not found for:", idOrTitle, "modules:", state.modules);
      return;
    }
  ...
  function openRandom() {
    ...
    if (!items || !items.length) return toast("Aucun item dans cette section.");
  ...
  function bindEvents() {
    ...
    if (el.modalMenu) el.modalMenu.onclick = () => {
      toast("Sommaire : version 1 (Précédent/Suivant).");
    };
    ...
    document.addEventListener("click", (e) => {
      ...
      if (back) { backToModules(); return; }

      const tab = t.closest("[data-tab]");
      if (tab) { state.tab = tab.getAttribute("data-tab"); renderModule(); return; }

      const rnd = t.closest("[data-random]");
      if (rnd) { openRandom(); return; }
